...
function removeLastOccurrences(str, value) {
  ...
  const { length } = value;
  return str.slice(0, lastIndex) + str.slice(lastIndex + length);
}
...
function formatTime(minutes, seconds) {
  ...
  return `${minutesToString.padStart(2, '0')}:${secondsToString.padStart(
    2,
    '0'
  )}`;
}
...
function isPalindrome(str) {
  ...
  return strWithoutSpace.toLowerCase() === strReverse.toLowerCase();
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  const reverseWordsInArr = arr.map((el) => el.split('').reverse().join(''));
  return reverseWordsInArr.join(' ');
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (str[i] === str[i].toUpperCase()) {
      currentAlphabet = alphabetUpperCase;
    }
    const currentLetterIndex = currentAlphabet.indexOf(str[i]);
    if (currentLetterIndex === -1) {
      result.push(str[i]);
    } else {
      const rot13LetterIndex =
        (currentLetterIndex + 13) % currentAlphabet.length;
      result.push(currentAlphabet[rot13LetterIndex]);
    }
  }
  return result.join('');
}
